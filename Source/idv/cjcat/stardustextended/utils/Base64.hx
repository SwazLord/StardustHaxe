...
class Base64 {
	...
	public static function encode(data:ByteArray):String {
		...
		out.length = (2 + data.length - Std.int(((data.length + 2) % 3) * 4 / 3)); // Preset length //1.6 to 1.5 ms
		var i:Int = 0;
		var r:Int = Std.int(data.length % 3);
		var len:Int = Std.int(data.length - r);
		var c:Int; // read (3) character AND write (4) characters
		var outPos:Int = 0;
		while (i < len) {
			// Read 3 Characters (8bit * 3 = 24 bits){

			c = Std.int(data[i++] << 16 | data[i++] << 8) | data[i++];

			out[outPos++] = _encodeChars[Std.int(c >>> 18)];
			out[outPos++] = _encodeChars[Std.int(Std.int(c >>> 12) & 0x3f)];
			out[outPos++] = _encodeChars[Std.int(Std.int(c >>> 6) & 0x3f)];
			out[outPos++] = _encodeChars[Std.int(c & 0x3f)];
		}
		...
		if (r == 1) // Need two "=" padding{
		{
			...
			c = data[i];
			out[outPos++] = _encodeChars[Std.int(c >>> 2)];
			out[outPos++] = _encodeChars[Std.int((c & 0x03) << 4)];
			out[outPos++] = 61;
			out[outPos++] = 61;
		} else if (r == 2) // Need one "=" padding{
		{
			c = Std.int(data[i++] << 8) | data[i];
			out[outPos++] = _encodeChars[Std.int(c >>> 10)];
			out[outPos++] = _encodeChars[Std.int(Std.int(c >>> 4) & 0x3f)];
			out[outPos++] = _encodeChars[Std.int((c & 0x0f) << 2)];
			out[outPos++] = 61;
		}
	...
	public static function decode(str:String):ByteArray {
		...
		while (i < len) {
			...
			c1 = _decodeChars[Std.int(byteString[i++])];
			if (c1 == -1) {
				break;
			}

			// c2
			c2 = _decodeChars[Std.int(byteString[i++])];
			if (c2 == -1) {
				break;
			}
