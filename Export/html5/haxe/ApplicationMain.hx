...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("StardustHaxe", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("StardustHaxe", null, 0, 0);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["file"] = "StardustHaxe";
		app.meta["name"] = "StardustHaxe";
		app.meta["packageName"] = "com.sample.stardusthaxe";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("Startup")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess Startup.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"Startup\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"Startup\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends Startup {}
#else
class DocumentClass
{
